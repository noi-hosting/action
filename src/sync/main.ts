...
export async function run(): Promise<void> {
  try {
    ...
    for (const migration of Object.values(migrations)) {
      ...
      await exec(
        `mysqldump -h ${migration.from.host} -u ${migration.from.user} --password=${migration.from.password} ${migration.from.name} > ${filename}`
      )
      await exec(
        `mysql -h ${migration.to.host} -u ${migration.to.user} --password=${migration.to.password} ${migration.to.name} < ${filename}`
      )
    }
